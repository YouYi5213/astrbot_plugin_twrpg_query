...
- 同步 `heros.json`：修正惩戒光环 `rawDesc` 文案
...
- 同步 TWRPGReader 最新离线数据：英雄技能描述、专属/被动数值、部分物品属性文案更新

---

### 改进

- 「读档器更新内容」增加别名：`读档器更新`、`读档器更新日志`
...
- 指令「读档器更新内容」：查询读档器最新版本与更新说明（不显示下载链接）
...
- 「世界存档」图片中主存档标记：emoji ⭐ 改为 ★，避免字体不支持时显示为方框叉号
...
- 「世界存档」：仅显示序号与存档名，去掉 `.txt` 后缀、文件大小与更新时间
- 「世界档案」：去掉徽章信息，存档名去掉 `.txt` 后缀
